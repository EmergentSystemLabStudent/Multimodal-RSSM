...
from common.models.encoder import ObsEncoder, MultimodalObsEncoder, StochasticStateModel, poe, mopoe
...
class MultimodalTransitionModel(nn.Module):
    ...
    def __init__(self, 
        ...
        self.obs_encoder = MultimodalObsEncoder(expert_dist=expert_dist, h_size=belief_size, s_size=state_size, activation=self.act_fn, embedding_sizes=embedding_sizes, hidden_size=hidden_size, min_std_dev=self.min_std_dev, device=device, use_prior_expert=use_prior_expert)
        self.modules += self.obs_encoder.modules
        self.use_prior_expert = use_prior_expert
        
        self.fusion_method = fusion_method
        if self.fusion_method == "MoPoE":
            self._get_posterior_states = mopoe
        else:
            self._get_posterior_states = poe
    ...
    def forward(self, prev_state: torch.Tensor, actions: torch.Tensor, prev_belief: torch.Tensor, observations: Optional[torch.Tensor] = None, nonterminals: Optional[torch.Tensor] = None, det=False) -> List[torch.Tensor]:
        ...
        for t in range(T - 1):
            ...
            if observations is not None:
                ...
                experts_loc = dict()
                experts_scale = dict()
                # if not self.use_prior_expert:
                #     experts_loc.append(prior_means[t + 1])
                #     experts_scale.append(prior_std_devs[t + 1])
                ...
                # posterior_mean, posterior_std_dev = self._get_posterior_states(experts_means, experts_std_devs)
                ...
                if det:
                    ...
                    # posterior_states[t + 1] = Normal(posterior_mean.squeeze(0), posterior_std_dev.squeeze(0)).rsample()
