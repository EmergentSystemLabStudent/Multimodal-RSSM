...
class MPCPlanner(jit.ScriptModule):
  ...
  def forward(self, belief, state):
    ...
    for _ in range(self.optimisation_iters):
      ...
      actions = (action_mean + action_std_dev * torch.randn(self.planning_horizon, B, self.candidates, self.action_size, device=action_mean.device)
                 ).reshape(self.planning_horizon, B * self.candidates, self.action_size)  # Sample actions (time x (batch x candidates) x actions)
      # Sample next states
      ...
      returns = self.reward_model(beliefs.reshape(-1, H), states.reshape(-1, Z)).reshape(
          self.planning_horizon, -1).sum(dim=0)  # output from r-model[12000]->view[12, 1000]->sum[1000]
  ...
  def get_action(self, belief, state, det=None):
    return self.forward(belief, state)
