...
def get_name_and_path(seed, itr, n_expert=5, n_uniform=5, n_data=100):
    ...
    #     date="10-01"
    date = "10-01"
    path = "../../MRSSM/MRSSM/results/{}/2022-{}/run_0/models_{}.pth".format(name, date, itr)
    return name, path
            ...
            for fix_flag in [True]:
                for itr in itrs:
                    for n_expert in [5]:
                        ...
                        n_uniform_rssm = 10 - n_expert_rssm
                        # n_expert_policy = n_expert
                        n_expert_policy = 5

                        _cfg = copy.deepcopy(cfg)
                        _cfg.main.test_interval = test_interval
                        _cfg.train.train_iteration = train_iteration
